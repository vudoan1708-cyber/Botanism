...
function showRecipe(recipe) {
    ...
    if (recipe.hits.length > 0) {
        ...
        childNodes.forEach(node => {
            node.style !== undefined ? (node.style.left = '-1000%', node.style.position = 'absolute') :
                console.log(node);
        });
...
function showRecipeNotification(recipe) {
    ...
    recipe !== null ?
        ...
        recipe.hits.length > 0 ? 
            ...
            toastr.success('Click here to see more', `There is ${recipe.hits.length} recipe(s) for this plant`, toastr.options) :
            toastr.warning(`There is ${recipe.hits.length} recipe(s) for this plant`, toastr.options) :

        // otherwise, recipe is a null obj
        toastr.danger('Cannot get the data at the moment', 'Error', toastr.options);
}
...
async function showPlantDetails(detail, img_url) {
    ...
    const childNodes = plant_detail.childNodes;
    childNodes.forEach(node => {
        node.style !== undefined ? (node.style.left = '0', node.style.position = 'relative') :
            console.log(node)
    });
